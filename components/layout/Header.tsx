...
function SearchInput({ className, onClose }: { className?: string; onClose?: () => void }) {
  ...
  const fetchSuggestions = useCallback(async (searchQuery: string, defaultSuggestions: SearchSuggestion[]) => {
    ...
    try {
      ...
      if (data.success && data.data?.length > 0) {
        const results: SearchSuggestion[] = data.data.slice(0, 6).map((gift: { title: string; slug: string; category: string }) => ({
          type: 'gift',
          title: gift.title,
          slug: gift.slug,
          icon: CATEGORIES.find(c => c.slug === gift.category)?.icon || '🎁',
        }));
  ...
  const handleSelect = (suggestion: SearchSuggestion) => {
    switch (suggestion.type) {
      case 'gift':
        router.push(`/gaver/${suggestion.slug}`);
        break;
      case 'age':
        router.push(`/alder/${suggestion.slug}`);
        break;
      case 'category':
        router.push(`/kategori/${suggestion.slug}`);
        break;
    }
    setIsOpen(false);
    setQuery('');
            ...
            {suggestions.map((suggestion, index) => (
              ...
              >
                ...
                <span className="ml-auto text-xs text-[#9CA3AF]">
                  {suggestion.type === 'gift' && 'Gave'}
                  {suggestion.type === 'age' && 'Alder'}
      ...
      >
        <div className="flex flex-col h-full">
          <div className="flex items-center justify-between p-4 border-b border-[#FFB5A7]/20">
            <span className="font-bold text-[#4A4A4A]">Menu</span>
            <button
              onClick={onClose}
...
export function Header() {
  ...
  const navLinks = [
    { href: '/gaver', label: 'Alle gaver', emoji: '🎁' },
    { href: '/kategori/legetoej', label: 'Legetøj', emoji: '🧸' },
    { href: '/konfirmation', label: 'Konfirmation', emoji: '🎓' },
